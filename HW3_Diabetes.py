...
max_depth=3                                         #max depth of gradient boosting classifier
...
if feat_select==1:
    ...
    if fs_type==2:
        ...
        if binning==1:
            clf = GradientBoostingClassifier(loss='deviance', learning_rate=0.1, n_estimators=100, max_depth=max_depth, min_samples_split=3, random_state=rand_st)
...
if binning==1 and cross_val==1:
    ...
    print('GB Max Depth: {}'.format(str(max_depth)))
    start_ts=time.time()
    clf = GradientBoostingClassifier(loss='deviance', learning_rate=0.1, n_estimators=100, max_depth=max_depth, min_samples_split=3, random_state=rand_st)
    ...
    scores_Acc = scores['test_Accuracy']                                                                                                                                    
    print("NN-MLP Classifier Acc: %0.2f (+/- %0.2f)" % (scores_Acc.mean(), scores_Acc.std() * 2))                                                                                                    
    scores_AUC = scores['test_roc_auc']                                                                     #Only works with binary classes, not multiclass                  
    print("NN-MLP Classifier AUC: %0.2f (+/- %0.2f)" % (scores_AUC.mean(), scores_AUC.std() * 2))                           
    print("NN-MLP Classifier CV Runtime:", time.time()-start_ts)
